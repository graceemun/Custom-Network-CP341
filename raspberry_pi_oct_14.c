...
//	}
	for (int i = q->front + 1; i < q->rear; i++){
		printf("PRINTED QUEUE: %c", q->items[i]);
	}
	printf("\n");
}
...
void *send_thread(){
	...
	/*
       	recipient_address = (int*)malloc(ADDRESS_SIZE * sizeof(int));
	recipient_address[0] = 1;
	recipient_address[1] = 0;
	recipient_address[2] = 1;
	recipient_address[3] = 0;
	recipient_address[4] = 1; */
	...
	while(running) {
		...
		if(strlen(user_input) > 199){
			...
			if(user_input[0] == 'w' && user_input[1] == '*'){
				...
				//recipient_address = realloc(recipient_address,ADDRESS_SIZE * sizeof(int));
...
void *receive_thread(){
	...
	while(run) {
		...
		//------------------------------------------------------------------------	
			...
			printf("\nFrom user %s: ", user);
		//------------------------------------------------------------------------	
			for(int z = 0; z < result_counter; z++){
