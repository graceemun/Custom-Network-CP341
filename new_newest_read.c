...
void binary_to_string(char* input, char* output){
	char binary[200] = {0}; //initialize string to 0

	//copy 8 bit from input string
	for(int i = 0; i < 8; i++){
		binary[i] = input[i];
	}
	*output = strtol(binary, NULL, 2); //convert the byte to long using base 2
}
...
*/
	...
	for(int i = 0; i < iterations; i++){
		binary_to_string(binary_res[i], text_translated[i]);
	}
	printf("MY OUTPUT: %c\n", text_translated);
