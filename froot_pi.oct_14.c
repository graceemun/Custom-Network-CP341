...
void *send_thread(){
	...
	printf("Who do you want to send your message to? Type 'g*' for r4sp_b3rry_pi or 'k*' for @pple_pi69. You do not need to press 'g*' or 'k*' continue chatting with the user you were previously chatting with. To switch users to chat with, press 'g*' or 'k*'. Type 'yy' to exit the program. Your message cannot exceed 200 characters max\n");
	...
	while(running) {
		...
		if(strlen(user_input) > 199){
			...
			if(user_input[0] == 'g' && user_input[1] == '*'){
				//choose recipient
				printf("Sending to r4sp_b3rry_pi\n");
			...
			for (int k = 0; k < strlen(result); k++){
				...
				//if the size is some short amount, wait a bit, and then send manchester
				if(strlen(result) <= 75){
					time_sleep(10);
				}
